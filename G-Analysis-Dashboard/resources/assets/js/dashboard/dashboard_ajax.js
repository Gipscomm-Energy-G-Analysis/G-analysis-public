    ...
    node.addEventListener("click", function () {
        ...
        for (let i = 0; i < numItems; i++) {
            let col = document.querySelector(
                ".column-name input.column-name_" + i
            ).id;
            result[i] = col;
        }
    ...
    if ((file1 = event.target.files[0])) {
        ...
        img.onload = function () {
            ...
            if (this.width <= 870 && this.height <= 621) {
                ...
                alert(
                    "Image dimensions should be smaller than or equal to  870 x 621."
                );
                return false;
            }
...
// $("#save_subgroup_options").on("change", function(e) {
//     let container = document.getElementById('dynamic_subgroup_field');
//     spinner.spin(container);
//     let group_id    = document.getElementById("group_id").value;
