...
function showexpandedchart() {
    ...
    inputs.each(function(){
        ...
        if(className.includes('chart_tile_expand_view')){
            ...
            $('.chart_tile_expand_view .'+id+'.edit_val.edit_btn_tile_chart').css('height','17px');
            $('.chart_tile_expand_view .'+id+'.edit_val.edit_btn_tile_chart').css('width','17px');
            $('.chart_tile_expand_view .'+id+'.edit_val.edit_btn_tile_chart').css('margin-right','5px');
...
function saveTableFormatMeasurementExpandView(){

    var row_enteries_length = $('#measurement_select_table_entries tr').length;
    var query_data = localStorage.getItem('query_data');
    ...
    // if(row_enteries_length <= 10 || measurementType == 'automatic'){
        ...
        // --end->
        // --end-->
        if(measurement_table_height != '' && measurement_table_width != ''){
            var measurement_preview_data = [];
            measurement_preview_data.push({'height':measurement_table_height,'width':measurement_table_width});
            localStorage.setItem('measurement_preview_data',JSON.stringify(measurement_preview_data));
        }
        ...
        if(row_enteries_length <= 10 || measurementType == 'automatic' ){
            $.ajax({
                ...
                data: {
                    ...
                    title : tile_title,
                    tile_html : tile_html,
                    height: measurement_table_height,
                    width : measurement_table_width,
                    tableHtml : tableHtml,
                    input_height : input_height,
                    input_width : input_width,
                    ...
                    if ($("#north-america-chart").length) {
                        ...
                        var northAmericaChartPlugins = {
                            beforeDraw: function(chart) {
                                ...
                                ctx.restore();
                                var fontSize = 3.125;
                                ctx.font = "570 " + fontSize + "em sans-serif";
    ...
    $.ajax({
        ...
        success: function(a) {
            ...
            if(input_width == "1" || input_width <= "0"){
                ...
                width_padding = 570; //285 * 2 + 30 Padding
            }
            else if(input_width == "3"){
                width_padding = 855;
            }
            ...
            var input_padding = '145';
            //Width Checks
            if(input_height == "1" || input_height <= "0"){
                input_padding = 145;
            }
            else if(input_height == "2"){
                input_padding = 320; //285 * 2 + 30 Padding
            }
            else if(input_height == "3"){
                input_padding = 495;
            }
            else if(input_height >= "4"){
                input_padding = 640;
            }
            localStorage.setItem('height',input_padding);
            //--end--->
            ...
            {
                // alert(a['data']['outside_tile_input_height']);
                // Height Checks
                var outside_tile_height = '';
                var outside_tile_width = '';
