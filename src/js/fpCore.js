    ...
    .freeze(
        new function () {
            ...
            this.doOperation = op => a => b => {
                const val1 = Number(String(a).replace(",", ".")),
                    val2 = Number(String(b).replace(",", "."));
                switch (op) {
                    case "(":
                        return this.opnPths(a, b); // opening parentheses
                        break;
                    case ")":
                        return this.clsPths(a, b); // closing parentheses
                        break;
                    case "/":
                        return this.devide(val1, val2);
                        break;
                    case "+":
            ...
            this.decode = this.id;
            this.isJSON = a => a.search(/\[[^\]]*\]/i) !== -1
            this.json = a => this.isJSON(a) ? JSON.parse(a) : [{
                json: "No valid JSON !",
                value: a
            }]
            ...
            this.deepFreeze = obj => {
                ...
                Object.getOwnPropertyNames(obj).forEach(prop => {
                    if (exists(obj)(prop) && obj[prop] !== null && (typeof obj[prop] === "object" || typeof obj[prop] === "function") && !Object.isFrozen(obj[prop])) {
                    ...
                    if (arr.length > 0) {
                        ...
                        const newAcc =
                            exists(selected_)("left") ? {
                                unselected: acc.unselected,
                                selected: push(acc.selected)(selected_.left)
                            } : {
                                unselected: push(acc.unselected)(selected_.right),
