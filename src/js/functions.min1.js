...
try {
    var dateTime = function(a) {
        ...
        messstellenInAuswertungsEditorTabelleEinlesen = function() {
            ...
            $.ajax({
                ...
                success: function(h) {
                    ...
                    for (var r = 0; r < q; r++) a = String(h[r].mst_ID), a = a.replace(/^\s+|\s+$/gm,
                        ""), b = String(h[r].nameMSt), b = "[" + b.replace(/^\s+|\s+$/gm, "") + "]", b = String(b.replace(/\s/g, "_")), e = String(h[r].kurzbezeichnungMst), e = e.replace(/^\s+|\s+$/gm, ""), c = String(h[r].energietraegerMst), c = c.replace(/^\s+|\s+$/gm, ""), g = String(h[r].messartMst), g = g.replace(/^\s+|\s+$/gm, ""), f = String(h[r].anlageMst), f = f.replace(/^\s+|\s+$/gm, ""), tblMessstellenBerechnungseditor.row.add([a + " ", b + " ", e + " ", c + " ", g + " ", f]).draw(), $("tr").css("cursor", "pointer")
                }
            })
        },
        ...
        dokumentAuswaehlenUndEinlesen = function(a) {
            a = a.target.files[0];
            var b = a.name,
            ...
            var c = !1,
                g = !1;
            if ("imgUploadAnl" == this.id) var f = "uploadsDownloads/php/uploadImage.php",
                c = !0,
                g = "Anl";
            else "imgUploadMsm" ==
                this.id ? (f = "uploadsDownloads/php/uploadImage.php", c = !0, g = "Msm") : "dokuAuswahlAnl" == this.id ? (e.append("id", $("#anlID").val()), e.append("verwaltung", "Anl"), e.append("kategorie", $("#kategorie").val()), f = "uploadsDownloads/php/upload.php") : "dokuAuswahlMsm" == this.id ? (e.append("id", $("#msmID").val()), e.append("verwaltung", "Msm"), e.append("kategorie", $("#kategorie").val()), f = "uploadsDownloads/php/upload.php") : "dokuAuswahlERng" == this.id && (e.append("id", $("#eRngID").val()), e.append("verwaltung", "ERng"), e.append("kategorie",
                    $("#kategorie").val()), f = "uploadsDownloads/php/upload.php");
            ...
            $.ajax({
                ...
                success: function(a) {
                    $(h).css("display", "none");
                    c ? $("Anl" == g ? "#bildAllgemeinAnl" : "#bildAllgemeinMsm").prop("src", "uploadsDownloads/images/" + $("#nameDB").val() + "/" + a) : dokumentenListeErstellen()
                }
            })
        },
        ...
        diagrammGruppeHinzuf\ u00fcgen = function() {
        ...
        mainMenuNav = function(a) {
            ...
            "erwPrdMenu" == a || "erwAnlMenu" == a || "entMenu" == a || "enfMenu" == a || "gsfMenu" == a || "mgsMenu" == a || "zpMenu" == a || "grpDiagMenu" == a ? ($("#tabsOptionen, #optionen").css("display",
                "block"), $("#tabsUnternehmensstruktur, #tabsEditor").css("display", "none"), $("#tabsBasisdaten").css("display", "block"), "entMenu" == a || "enfMenu" == a ? tabControlNav("tabEng") : "gsfMenu" == a ? tabControlNav("tabGsf") : "mgsMenu" == a ? tabControlNav("tabMgs") : "zpMenu" == a ? tabControlNav("tabZp") : "erwAnlMenu" == a ? tabControlNav("tabEAnl") : "erwPrdMenu" == a && tabControlNav("tabEPrd")) : "grpDiagMenu" == a ? tabControlNav("tabGrpDiag") : $("#tabsOptionen, #optionen").css("display", "none");
