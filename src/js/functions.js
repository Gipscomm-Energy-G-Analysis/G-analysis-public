...
try {
    ...
    function chartInNewWindow2() {
        ...
        if(timeInterval==="Benutzerdefiniert"){
            queryString_1 = "SELECT nameMSt AS Name, CONVERT(varchar(20), time_de, 104) + ' ' + CONVERT(varchar(20), time_de, 108) AS Time, phase AS Phase, " + valName + " AS Value, wandlungsfaktorMsm AS ConvFactor FROM data_value_15m INNER JOIN channel ON data_value_15m.channel_id = channel.channel_id INNER JOIN messmittel ON data_value_15m.channel_id = messmittel.kanal1Msm OR data_value_15m.channel_id = messmittel.kanal2Msm OR data_value_15m.channel_id = messmittel.kanal3Msm INNER JOIN messstellen ON messmittel.mst_ID = messstellen.mst_ID WHERE messstellen.mst_ID = '" + idMst + "' AND time_de >= '" + von1 + "' AND time_de < '" + bis1 + "' ORDER by time_de";
            queryString_2 = "SELECT nameMSt AS Name, CONVERT(varchar(20), time_de, 104) + ' ' + CONVERT(varchar(20), time_de, 108) AS Time, phase AS Phase, " + valName + " AS Value, wandlungsfaktorMsm AS ConvFactor FROM data_value_15m INNER JOIN channel ON data_value_15m.channel_id = channel.channel_id INNER JOIN messmittel ON data_value_15m.channel_id = messmittel.kanal1Msm OR data_value_15m.channel_id = messmittel.kanal2Msm OR data_value_15m.channel_id = messmittel.kanal3Msm INNER JOIN messstellen ON messmittel.mst_ID = messstellen.mst_ID WHERE messstellen.mst_ID = '" + idMst + "' AND time_de >= '" + von2 + "' AND time_de < '" + bis2 + "' ORDER by time_de";
            queryString_3 = "SELECT nameMSt AS Name, CONVERT(varchar(20), time_de, 104) + ' ' + CONVERT(varchar(20), time_de, 108) AS Time, phase AS Phase, " + valName + " AS Value, wandlungsfaktorMsm AS ConvFactor FROM data_value_15m INNER JOIN channel ON data_value_15m.channel_id = channel.channel_id INNER JOIN messmittel ON data_value_15m.channel_id = messmittel.kanal1Msm OR data_value_15m.channel_id = messmittel.kanal2Msm OR data_value_15m.channel_id = messmittel.kanal3Msm INNER JOIN messstellen ON messmittel.mst_ID = messstellen.mst_ID WHERE messstellen.mst_ID = '" + idMst + "' AND time_de >= '" + von3 + "' AND time_de < '" + bis3 + "' ORDER by time_de";
        }
    ...
    bereicheInDropbox = function (a) {
        ...
        channelNameInDropbox = function (a) {
            $("#channelNameDropBox").empty();
                for (i = 0; i < a.length; i++) $("#channelNameDropBox").append("<option value='"+ a[i].channel_id +"'>" + a[i].name + "</option>");
                $("#channelNameDropBox").append('<option value="other">Other</option>');
                $("#newChannelName").prop("disabled", true);
        },
        channelNameEinlesen = function () {
            $.ajax({
                type: "POST",
                async: true,
                url: "php/getChannelNameListInDropbox.php",
                data: {
                    nameDB: $("#nameDB").val()
                },
                success: function (a) {
                    a = JSON.parse(a);
                    channelNameInDropbox(a)
                }
            })
        },
        unitNameInDropbox = function (a) {
            $("#unitID").empty();
                for (i = 0; i < a.length; i++) $("#unitID").append("<option>" + a[i].unit_id + "</option>");
        },
        unitNameEinlesen = function () {
            $.ajax({
                type: "POST",
                async: true,
                url: "php/getUnitNameListInDropbox.php",
                data: {
                    nameDB: $("#nameDB").val()
                },
                success: function (a) {
        ...
        mandantenInDropbox = function (a) {
            ...
            ["msm", "std", "anl", "channel", "ent", "enf", "eRng", "iMw", "zp"]
            ...
            $.ajax({
                ...
                success: function (a) {
                    ...
                    JSON.parse(a).map(function (a) {
                        return [a.gDia_ID, a.name, a.beschreibung, a.typ, JSON.parse(a.jsonDiag).map(b), a.bemerkung, '<img id="'+a.gDia_ID+'" class="deleteSaveGraph" title="Delete" src="images/delete.png" style="height:12px;width:12px;margin-left:10px;">']
    ...
    bereichelisteErstellen = function () {
        ...
        }, channellisteErstellen = function () {
            $.ajax({
                type: "POST",
                async: !0,
                url: "php/getChannelListTbl.php",
                data: {
                    nameDB: $("#nameDB").val()
                },
                success: function (a) {
                    a = JSON.parse(a);
                    tblChannelSuchen.colReorder.reset();
                    tblChannelSuchen.clear().draw();
                    for (var b = 0; b < a.length; b++) tblChannelSuchen.row.add([a[b].channel_id, a[b].name, a[b].unit_id, a[b].number, a[b].SignalName, a[b].phase, a[b].scaleend, a[b].resolution]).draw();
                    tblChannelSuchen.column(0).visible(!1);
                    $("#channelSuchenContainer").css("display", "block");
                    $("#channelSuchenContainer").dialog({
                        height: $(window).height() - .125 * $(window).height(),
                        width: $(window).width() - .125 * $(window).width(),
                        resize: "auto",
                        show: {
                        ...
                        open: function () {
                            $("#tblChannelSuchen tbody tr").css("cursor", "pointer");
                            $("#tblChannelSuchen tbody").on("dblclick", "tr", function () {
                                clearFields("channelHinz");
                                $(".newChannelName").hide();
                                $("#newChannelName").prop("disabled", true).prop("checked", false);
                                var a = tblChannelSuchen.row(this).data();
                                $("#channelSuchenContainer").dialog("close");
                                //readInstanzen("channelFirst", a[0])
                                0 < a.length ? (    
                                $("#channelID").val(a[0]),
                                $("#channelNameDropBox option").each(function () {
                                    if ($(this).text().trim() === a[1].trim()) {
                                        $(this).prop("selected", true);
                                        return false; // Stop loop once match is found
                                    }
                                }),
                                $("#unitID").val(a[2]),
                                $("#channelNumber").val(a[3]),
                                $("#signalName").val(a[4]),
                                $("#channelPhase").val(a[5]),
                                $("#scaleend").val(a[6]),
                                $("#resolution").val(a[7])
                                ) : clearFields("channelHinz")
                            })
        ...
        checkboxenDerEntEnfAlleZuruecksetzen = function () {
            ...
            "gipscAdmHinz" == a ? b = ["#benutzernameGipscAdm", "#passwortGipscAdm"] : "betrGrpHinz" == a ? b = "#firmaBetrGrp #anzahlMitarbeiterBetrGrp #anschriftBetrGrp #plzBetrGrp #ortBetrGrp #geschaeftsfuehrerBetrGrp #telefonBetrGrp #emailBetrGrp #notizBetrGrp".split(" ") :
                ...
                "channelHinz" == a ? b = "#channelNameDropBox #unitID #channelName #channelNumber #signalName #channelPhase #scaleend #resolution".split(" ") :
            ...
            switch (isInstance(a)) {
                ...
                case "channel":
                    $.ajax({
                        ...
                        success: function (a) {
                            var c = JSON.parse(a);
                            $("#channelCount").val(c.length);
                            0 < c.length ? (
                                [
                                $("#channelNameDropBox option").each(function () {
                                let channelName=c[b].name;
                                    if ($(this).text().trim() === channelName.trim()) {
                                        $(this).prop("selected", true);
                                        return false; // Stop loop once match is found
                                    }
                                }),
    ...
    instanzSpeichern = function (a) {
            ...
            }else if ("channelSpeichern" == a) {
                let checkbox = document.getElementById("newChannelName");
                let channelName='';
                let modus ='';
                let channel_ID ='';
                    if (checkbox.checked) {
                        ...
                        channelName= $("#channelNameDropBox option:selected").text();
                        modus ="update";
                        channel_ID =$("#channelNameDropBox option:selected").val();
                    }
                    $.ajax({
                        type: "POST",
                        async: !0,
                        ...
                        data: {
                            ...
                            unitID: $("#unitID option:selected").text(),
                            channelNumber: $("#channelNumber option:selected").text(),
                            signalName: $("#signalName").val(),
                            phase: $("#channelPhase option:selected").text(),
                            scaleend: $("#scaleend").val(),
                            resolution: $("#resolution").val()
                        },
                        success: function (a) {
                             alert(datensatzGespeichert(a));
                             channelNameEinlesen();
                             readInstanzen("channelLast", $("#channelCount").val())
                             $(".newChannelName").hide();
                        }
                    });
            ...
            for (var e = 0; e < nTabsData; e++)
                if (a == tabsData[e].tab && "" != tabsData[e].info) {
                    ...
                    else if ("tabChannel" == a) $("#channel").css("display", "block"), $("#infosChannel").css("display", "block");
            ...
            if ("channel_Menu" == a) {
                ...
                tabControlNav(b)
            } else $("#tabsChannel, #channel").css("display","none");
    ...
    tblChannelSuchen = $("#tblChannelSuchen").DataTable({
        dom: "Bfrtip",
        buttons: [{
            extend: "copy",
            text: "Kopieren",
    ...
    $.ajax({
        ...
        success: function (a) {
            ...
            if(data.error){
                ...
                tblGespDiagrammeListe.clear().draw();
            }else{
                tblGespDiagrammeListe.clear();
                JSON.parse(a).map(function (a) {
                    var moduleType='';
    ...
    if (!result) {
        ...
        $.ajax({
            ...
            success: function (a) {
                ...
                savedGraphListing(sessionStorage.getItem("saved_graph_popup_module"));
            }
        })
    }
});
