...
$jscomp.polyfill("Promise", function(a) {
    ...
    c.prototype.executeOnSettledCallbacks_ = function() {
        if (null != this.onSettledCallbacks_) {
            for (var a = this.onSettledCallbacks_, b = 0; b < a.length; ++b) a[b].call(),
                a[b] = null;
    ...
    c.race = function(a) {
        return new c(function(b, e) {
            for (var g = $jscomp.makeIterator(a), f = g.next(); !f.done; f = g.next()) c.resolve(f.value).callWhenSettled_(b, e)
        })
    };
    c.all = function(a) {
        ...
        return e.done ? c.resolve([]) : new c(function(a, g) {
            ...
            var h = [],
                q = 0;
            do h.push(void 0), q++, c.resolve(e.value).callWhenSettled_(f(h.length - 1), g), e = b.next(); while (!e.done)
        })
    };
...
$jscomp.polyfill("Array.from", function(a) {
    return a ? a : function(a, e, c) {
        ...
        if ("function" == typeof f)
            for (a = f.call(a); !(f = a.next()).done;) b.push(e.call(c, f.value));
        else
            for (var f = a.length, h = 0; h < f; h++) b.push(e.call(c, a[h]));
        return b
    }
}, "es6-impl", "es3");
...
try {
    var dateTime = function(a) {
        ...
        chartInNewWindowKnz = function() {
            ...
            sessionStorage.setItem("knzID_2",
                $("#knzIDDiag2").val());
        ...
        toggleStandorteDritter = function(a) {
            ...
            $.ajax({
                ...
                success: function(b) {
                    ...
                    for (var c = 0; c < b.length; c++) tblMessstelleAuswahl.row.add([b[c].nameMSt, b[c].kurzbezeichnungMst, b[c].kostenstelleMst, b[c].messmittelBerechnungslogikMst, b[c].mst_ID]).draw();
                    ...
                    $("#messstellenAuswahlContainer").dialog({
                        ...
                        open: function() {
                            $("#tblMessstellenlisteMst tbody tr").css("cursor",
                                "pointer");
                            ...
                            $("#tblMessstellenlisteMst tbody").on("dblclick", "tr", function() {
                                ...
                                else if ("mstDiag1" == a) $("#mstDiag1").val(b[0]), $("#mstIDDiag1").val(b[4]);
                                else if ("mstDiag2" == a) $("#mstDiag2").val(b[0]), $("#mstIDDiag2").val(b[4]);
                                else if ("mstDiag3" == a) $("#mstDiag3").val(b[0]), $("#mstIDDiag3").val(b[4]);
                    ...
                    $.ajax({
                        ...
                        success: function(a) {
                            ...
                            0 < c.length ? ($("#bildAllgemeinAnl").prop("src", c[b].bildAnl), $(".anlageAnl").text(c[b].bezeichnungAnl), $("#aktivAllgemeinAnl").prop("checked", c[b].aktivAnl), $("#mehrProdukteAllgemeinAnl").prop("checked", c[b].mehrProdukteAnl), [
                                    ...
                                    ["#anschlussleistung1Anl", "anschlussleistung1Anl"],
                                    ["#mittlereAuslastungProzent1Anl", "mittlereAuslastungProzent1Anl"],
                                    ["#mittlereAuslastungKw1Anl", "mittlereAuslastungKw1Anl"],
                                    ...
                                    ["#energietraeger2AllgemeinAnl",
                                        "energietraeger2Anl"
                                    ],
                                    ["#energieform2AllgemeinAnl", "energieform2Anl"],
                                    ["#einheit2Anl", "einheitEnergie2Anl"],
                                    ["#anschlussleistung2Anl", "anschlussleistung2Anl"],
                                    ["#mittlereAuslastungProzent2Anl", "mittlereAuslastungProzent2Anl"],
                                    ["#mittlereAuslastungKw2Anl", "mittlereAuslastungKw2Anl"],
                                    ...
                                    ["#anschlussleistung3Anl", "anschlussleistung3Anl"],
                                    ["#mittlereAuslastungProzent3Anl", "mittlereAuslastungProzent3Anl"],
                                    ["#mittlereAuslastungKw3Anl", "mittlereAuslastungKw3Anl"],
                                    ...
                                    ["#anschlussleistung4Anl", "anschlussleistung4Anl"],
                                    ["#mittlereAuslastungProzent4Anl", "mittlereAuslastungProzent4Anl"],
                                    ["#mittlereAuslastungKw4Anl", "mittlereAuslastungKw4Anl"],
...
function dynamischeKorrekturfaktorenSpeichern() {
    ...
    for (i = 0; i < $("#tblOptionenEPrdDKff tbody tr").length; i++) {
        ...
        optionFaktore[i] = tblOptionenEPrdDKff.cell(i, 3).data();
        optionBasisFaktorName[i] = tblOptionenEPrdDKff.cell(i, 4).data();
        optionBasisFaktorCalc[i] = tblOptionenEPrdDKff.cell(i, 5).data();
        optionBasisFaktorWert[i] = tblOptionenEPrdDKff.cell(i, 6).data();
        optionBasisResult[i] = tblOptionenEPrdDKff.cell(i, 7).data();
        xOptionName = optionName;
...
function getDynamischeKorrekturfaktoren(id) {
    $.ajax({
        ...
        success: function(a) {
            ...
            for (var e = 0; e < b; e++)
                tblGetDyanamicheKorrekturfaktoren.row.add(
                    [a[e].subtypeTxtOptNameDKff,
                        ...
                        a[e].faktorWert,
                        a[e].result,
                        "<a data-id='" + a[e].dKffOption_id + "' data-id-parent='" + a[e].dKff_id + "' class='dyanamicheKorrekturfaktorenMenuEdit'>Edit</a> | <a class='dyanamicheKorrekturfaktorenMenuDel' data-id='" + a[e].dKffOption_id + "'>Delete</a>"
                    ]).draw(),
                $("tr").css("cursor", "pointer")
        }
    })
        ...
        success: function(e) {
            ...
            for (var c = 0; c < e.length; c++) {
                var rowNode = tblGetDyanamicheKorrekturfaktorenParent.row.add([
                    ...
                    e[c].basisType,
                    e[c].typeDynamicCF,
                    e[c].subtypeTimeDynamicCF
                ]).draw().node();
                $(rowNode).attr('data-id', e[c].dKff_id).css("cursor", "pointer");
            }
    ...
    $.ajax({
        ...
        success: function(a) {
            ...
            if (a.length == 1) {
                ...
                if (basisType == 'Basis Faktor') {
                    $(".subtypeTxtDynamicCF").show();
                    $(".subtypeTxtBasisFaktor2").hide();
                    $(".subtypeTxtBasisFaktor3").hide();
                    var column = tblOptionenEPrdDKff.columns([3, 4, 5, 6]);
                    column.visible(!column.visible());
                    $('#tblGetDyanamicheKorrekturfaktoren').parents('div.dataTables_wrapper').first().hide();
                    $('#tblOptionenEPrdDKff').parents('div.dataTables_wrapper').first().hide();

                } else if (basisType == 'Basis Faktor + 2. Bedingung') {
                    $(".subtypeTxtDynamicCF").show();
                ...
                } else if (basisType == 'Basis Faktor + 3. Bedingung') {
                    $(".subtypeTxtDynamicCF").show();
                    $(".subtypeTxtBasisFaktor2").show();
