    ...
    function (a) {
        ...
        c.race = function (a) {
            return new c(function (b, d) {
                for (var g = $jscomp.makeIterator(a), f = g.next(); !f.done; f = g.next()) c.resolve(f.value).callWhenSettled_(b, d);
            });
        };
        c.all = function (a) {
            ...
            return d.done
                ...
                : new c(function (a, f) {
                      ...
                      var e = [],
                          l = 0;
                      do e.push(void 0), l++, c.resolve(d.value).callWhenSettled_(g(e.length - 1), f), (d = b.next());
                      while (!d.done);
                  });
        };
        $jscomp.EXPOSE_ASYNC_EXECUTOR &&
...
var scpFormula = freeze(
    new (function () {
        var a = freeze({ kpis: "php/kpiRecord.php", energyData: "php/energyData.php", prodData: "php/prodData.php" }),
            d = function (a) {
                return function (b) {
                    return apply(b)(a);
                };
            ...
            k = function (a) {
                return 0 < ["60min", "30min", "15min", "5min"].filter(equal(a)).length;
            };
        this.datasetRaw = function (a) {
            return function (e) {
                return function (g) {
                    return c({ nameDB: a, knzID: e }).then(function (c) {
                        return d({ nameDB: a, ins: c.bezug, id: c[c.bezug + "_ID"], nRecords: g })(k(head(get(f)(c.idString))) ? h : b);
                    });
                };
            };
        };
        this.datasetFormula = function (a) {
            return function (e) {
                return function (g) {
                    return c({ nameDB: a, knzID: e })
                        .then(function (c) {
                            console.log('c+spies',c);
                            let ddata = d({ nameDB: a, ins: c.bezug, id: c[c.bezug + "_ID"], nRecords: g });
                            let kdata = k(head(get(f)(c.idString)));
                            console.log('firstD',ddata);
                            console.log('firstK',kdata);
                            console.log('firsth',h);
                            console.log('firstb',b);
                            console.log('firstc',c);
                            let data = [d({ nameDB: a, ins: c.bezug, id: c[c.bezug + "_ID"], nRecords: g })(k(head(get(f)(c.idString))) ? h : b), c];
                            console.log('data+spies',data);
                            return data;
                        })
                        .then(function (a) {
                            console.log('a+spies',a)
                            return a[0]
                                .map(function (b) {
                                    let data = atob(a[1].idString)
                                        .split(" ")
                                        .map(function (a) {
                                            var c = a.split("_");
                                            let cString = c[0].toString();
                                            return  equal("bdeprod")("bdeprod") ? b[a.split("-")[1]] : equal(c[0])("mst") && equal(c[1])(b.mst_ID) ? b.Value : equal(c[0])("anl") && equal(c[1])(b.anl_ID) ? b.Value : a;
                                        });
                                    console.log('b__data',data);
                                    return data;
                                })
                                .map(function (a) {
                                    
                                    let data = a.map(function (a) {
                                        return String(a).replace(",", ".");
                                    });
                                    console.log('data_third_map',data);
                                    return data;
                                })
                                .map(function (a) {
                                    let data = a.reduce(function (a, b) {
                                        return a + " " + b;
                                    });
                                    console.log('data_forth_map',data);
                                    return data;
                                })
                                .map(function (a) {
                                    console.log('data_fifth_map',a);
                                    let data = math.eval(String(a));
                                    console.log('data_fifth_map',data);
                                    return data;
                                });
                        });
                };
