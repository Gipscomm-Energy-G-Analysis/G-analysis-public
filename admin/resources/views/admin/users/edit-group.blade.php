...
<div class="row">
    <div class="container-fluid">
        <form action="{{ route('update-group') }}" method="POST" id="edit-group">
            @csrf
            <div class="card card-secondary">
                ...
                <div class="card-body p-0">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <textarea id="description" placeholder="{{ __('groups.Description') }}" class="form-control" rows="1" name="description">{{$data->description}}</textarea>
                                        @error('description')
                        ...
                        <div class="card-body">
                            ...
                            <input class="role-permission-ids" type="hidden" value="{{$GroupMenuPermissions}}"
                                name="rolePermissionIds">
                                <input class="role-permission-addIds" type="hidden" value="{{$GroupMenuPermissions}}"
                                name="rolePermissionAddIds">
                                <input class="role-permission-editIds" type="hidden" value="{{$GroupMenuPermissions}}"
                                name="rolePermissionEditIds">
                                <input class="role-permission-deleteIds" type="hidden" value="{{$GroupMenuPermissions}}"
                                name="rolePermissionDeleteIds">
                        </div>
                    </div>
                </div>
                <div class="card-sec-submit d-flex justify-content-end">
                <a href="{{ route('group-list') }}"><button type="button" class="btn btn-default mr-2">{{ __('groups.cancel') }}</button></a>
...
function treeViewData() {
    var treeViewData = @json($treeGroupMenuPermission);
    return treeViewData;
}
//var selectedItemIds = [5,55,56,57,58,59,60,61,62,63,64,65];
function selectedItemMenuIds(){
    var selectedItemIds = [{{$GroupMenuPermissions}}];
...
var artaraxTreeView = $.artaraxTreeView({
    ...
    selectedIds: selectedItemMenuIds(), // just use on update mode (when you run tree view by 'loadTreeViewOnUpdate()' function)
    // addCallBack: onAdd,
...
// use above line on insert mode, for update mode use below line
 artaraxTreeView.loadTreeViewOnUpdate("#");
...
///////Menu////
var selectedItemIds = [{{$GroupMenuPermissions}}];
$.each(selectedItemIds,function(i){
var id =selectedItemIds[i];
   $('#menuchk_'+id).prop('checked', true);
});
///////add////
var addSelectedItemIds = [{{$addGroupMenuPermissions}}];
$.each(addSelectedItemIds,function(i){
var id =addSelectedItemIds[i];
   $('#addchk_'+id).prop('checked', true);
});
//////edit////
var editSelectedItemIds = [{{$editGroupMenuPermissions}}];
$.each(editSelectedItemIds,function(i){
var id =editSelectedItemIds[i];
   $('#updatechk_'+id).prop('checked', true);
});
///////delete////
var deleteSelectedItemIds = [{{$deleteGroupMenuPermissions}}];
$.each(deleteSelectedItemIds,function(i){
var id =deleteSelectedItemIds[i];
   $('#deletechk_'+id).prop('checked', true);
});
