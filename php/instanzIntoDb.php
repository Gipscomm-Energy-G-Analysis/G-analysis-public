...
elseif($id == "mstE" || $id == "mstB") {
    ...
    $modus = $_POST['modus'] ;
    $typ = $id === "mstE" ? "energiedaten" : "betriebsdaten" ;
    $messmittelBerechnungslogik = $_POST['messmittelBerechnungslogik'] ;
    $messmittelBerechnungslogikMstNormal = 'NULL';
    $messmittelBerechnungslogikMst='';
    if(!empty($_POST['messmittelBerechnungslogik']) && $_POST['messart']=="berechnet"){
      $messmittelBerechnungslogikMst = base64_encode($_POST['messmittelBerechnungslogik']);
      $messmittelBerechnungslogik = base64_encode($_POST['messmittelBerechnungslogik']) ;
      $messmittelBerechnungslogikMstNormal = $_POST['messmittelBerechnungslogik'];
    }
    ...
    if ($modus == "new"){
    	...
    	$tsql = "INSERT INTO messstellen(ber_ID,datumMst,nameMSt,kurzbezeichnungMst,kostenstelleMst,aktivMst,energietraegerMst,energieformMst,ortMst,messartMst,vorgelMst_ID,messmittelBerechnungslogikMst,notizMst,messmittelBerechnungslogikMstNormal, deleted, anl_ID, msm_ID, extDl_ID, isDurchleitung, beschreibung, typ) " ;
        $tsql .= "VALUES ('$berID',getdate(),'$messstellenbezeichnung',' $kurzbezeichnung','$kostenstelle','$aktiv','$energietraeger','$energieform','$ort','$messart','$vorgelMstID','$messmittelBerechnungslogik','$notiz','$messmittelBerechnungslogikMstNormal', 0, '$anlID', '$msmID', '$extDlID', '$isDurchleitung', '$beschreibung', '$typ') " ;
    }
    else {
    	$mstID = $_POST['mstID'] ;

      $oldMstName =   "SELECT nameMSt FROM MessstellenAnlagen WHERE mst_ID = '$mstID'";
      $getoldMstName = queryDB($conn, $oldMstName, "read");
      $oldMstnameValue = $getoldMstName[0]['nameMSt'];

      //update mst name in tiles table
      if(!empty($mstID) && !empty($messstellenbezeichnung) && !empty($oldMstnameValue)){
      $MstNameTile =  "UPDATE tableFormat SET tile_html = REPLACE(CONVERT(VARCHAR(MAX), tile_html), '$oldMstnameValue', '$messstellenbezeichnung') WHERE tile_html LIKE '%$oldMstnameValue%'";
        queryDB($conn, $MstNameTile, "write");
      }
        ...
        $tsql .= "messmittelBerechnungslogikMst = '$messmittelBerechnungslogik', notizMst = '$notiz',messmittelBerechnungslogikMstNormal = '$messmittelBerechnungslogikMstNormal', ortMst = '$ort', energieformMst = '$energieform', extDl_ID = '$extDlID', isDurchleitung = '$isDurchleitung', beschreibung = '$beschreibung' " ;
    	$tsql .= "WHERE mst_ID = '$mstID' " ;

      //start update mst name in saved graph json
      if(!empty($mstID) && !empty($messstellenbezeichnung)){
        $savedgraphMST1 =  "UPDATE gespeicherteGraphDiagramme SET jsonDiag = JSON_MODIFY(jsonDiag, '$.nameMst_1', '$messstellenbezeichnung') WHERE JSON_VALUE(jsonDiag, '$.mstID_1') = '$mstID' AND JSON_VALUE(jsonDiag, '$.nameMst_1') IS NOT NULL";
        queryDB($conn, $savedgraphMST1, "write");
      
        $savedgraphMST2 =  "UPDATE gespeicherteGraphDiagramme SET jsonDiag = JSON_MODIFY(jsonDiag, '$.nameMst_2', '$messstellenbezeichnung') WHERE JSON_VALUE(jsonDiag, '$.mstID_2') = '$mstID' AND JSON_VALUE(jsonDiag, '$.nameMst_2') IS NOT NULL";
        queryDB($conn, $savedgraphMST2, "write");
      
        $savedgraphMST3 =  "UPDATE gespeicherteGraphDiagramme SET jsonDiag = JSON_MODIFY(jsonDiag, '$.nameMst_3', '$messstellenbezeichnung') WHERE JSON_VALUE(jsonDiag, '$.mstID_3') = '$mstID' AND JSON_VALUE(jsonDiag, '$.nameMst_3') IS NOT NULL";
        queryDB($conn, $savedgraphMST3, "write");
      
        $savedgraphMST4 =  "UPDATE gespeicherteGraphDiagramme SET jsonDiag = JSON_MODIFY(jsonDiag, '$.nameMst', '$messstellenbezeichnung') WHERE JSON_VALUE(jsonDiag, '$.mstID') = '$mstID' AND JSON_VALUE(jsonDiag, '$.nameMst') IS NOT NULL";
        queryDB($conn, $savedgraphMST4, "write");
      
        $savedgraphMST5 =  "UPDATE gespeicherteGraphDiagramme SET jsonDiag = JSON_MODIFY(jsonDiag, '$.nameMst', '$messstellenbezeichnung') WHERE JSON_VALUE(jsonDiag, '$.mstID_1') = '$mstID' AND JSON_VALUE(jsonDiag, '$.nameMst') IS NOT NULL";
        queryDB($conn, $savedgraphMST5, "write");
      }
      //end update mst name in saved graph json
    }
}
...
//strat code save channel Author KP
elseif($id == "channel") {
  $modus = $_POST['modus'];
  ...
  if($modus == "new" || $channelID ==""){
    $tsql = "INSERT INTO channel(name, label, description, unit_id, number, SignalName, phase, scaleend, resolution) ";
    $tsql .= "VALUES ('$channelName','$channelName','$channelName','$unitID', '$channelNumber', '$signalName', '$phase', '$scaleend', '$resolution') ";
  }
  else{
  $tsql = "UPDATE channel SET name = '$channelName', label = '$channelName', description = '$channelName', unit_id = '$unitID', number = '$channelNumber', SignalName = '$signalName', phase = '$phase', scaleend = '$scaleend', resolution = '$resolution' ";
  $tsql .= "WHERE channel_id = '$channelID' ";
  }
  }
//end code save channel Author KP  
elseif($id == "ePrd") {
$modus = $_POST['modus'];
$name = $_POST['name'];
$kurz = $_POST['kurz'];
